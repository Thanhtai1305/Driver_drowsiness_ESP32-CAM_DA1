...
bool detectFaceAndEyes(camera_fb_t *fb, int *face_x, int *face_y, int *face_size) {
    ...
    if (max_contrast < 10.0f) {
        Serial.println("Khong tim thay mat!");
        return false;
    }

    Serial.print("Vung mat tai: (");
    Serial.print(*face_x);
    Serial.print(", ");
    Serial.print(*face_y);
    Serial.print(") voi kich thuoc: ");
...
bool detectEyesInFace(uint8_t *src, int width, int height, int face_x, int face_y, int face_size, int *left_eye_x, int *left_eye_y, int *right_eye_x, int *right_eye_y) {
    ...
    if (left_candidates[0].edge_value < 30.0f || right_candidates[0].edge_value < 30.0f) {
        Serial.println("Canh qua yeu, khong tim thay mat!");
        return false;
    }
    ...
    *right_eye_y = right_candidates[0].y;
    ...
    Serial.print("Goc nghieng cua mat: ");
    Serial.print(tilt_angle);
    Serial.println(" do");

    if (eye_distance < min_eye_distance) {
        Serial.println("Khoang cach giua hai mat qua gan!");
        ...
        *right_eye_y = mid_y + (adjusted_distance / 2) * sin(angle_rad);
        ...
        Serial.println("Khoang cach giua hai mat qua xa!");
    ...
    Serial.print("Mat trai tai: (");
    Serial.print(*left_eye_x);
    Serial.print(", ");
    Serial.print(*left_eye_y);
    Serial.println(")");
    Serial.print("Mat phai tai: (");
...
float processImage(camera_fb_t *fb, TfLiteTensor* input, int left_eye_x, int left_eye_y, int right_eye_x, int right_eye_y) {
    int crop_size = 24;
    int dst_size = 24;

    float tilt_angle = atan2(right_eye_y - left_eye_y, right_eye_x - left_eye_x) * 180 / PI;

    cropAndResize(workingImage, processedImage, 96, 96, left_eye_x, left_eye_y, crop_size, dst_size, tilt_angle);
    uint8_t temp[24 * 24];
    memcpy(temp, processedImage, 24 * 24);
    smoothImage(temp, processedImage, 24, 24);

    for (int i = 0; i < 24 * 24; i++) {
        input->data.f[i] = (float)processedImage[i] / 255.0f;
    }
    float left_eye_prob = runModel();
    if (left_eye_prob < 0) left_eye_prob = 0.5f;

    cropAndResize(workingImage, processedImage, 96, 96, right_eye_x, right_eye_y, crop_size, dst_size, tilt_angle);
    memcpy(temp, processedImage, 24 * 24);
    smoothImage(temp, processedImage, 24, 24);
    ...
    float right_eye_prob = runModel();
    if (right_eye_prob < 0) right_eye_prob = 0.5f;

    float avg_prob = (left_eye_prob + right_eye_prob) / 2.0f;

    Serial.println("Mot so gia tri pixel sau xu ly (mat phai):");
    for (int i = 0; i < 10; i++) {
...
void loop() {
    ...
    if (detectFaceAndEyes(fb, &face_x, &face_y, &face_size)) {
        ...
        if (detectEyesInFace(workingImage, 96, 96, face_x, face_y, face_size, &left_eye_x, &left_eye_y, &right_eye_x, &right_eye_y)) {
            TfLiteTensor* input = interpreter->input(0);
            float eye_open_prob = processImage(fb, input, left_eye_x, left_eye_y, right_eye_x, right_eye_y);
